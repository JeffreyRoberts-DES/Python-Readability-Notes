...
Variables store a value in memory under a name so it can be reused.
...
| Type | Meaning |
|---|---|
| `str` | Text/letters. Concatenating strings joins them — no math is performed. |
| `int` | Numeric values without quotes. |
...
h = None     # NoneType
i = ""       # str - Blank
j = " "      # str - Whitespace (not the same as blank or None!)
```

> **Note:** `""` (blank) is still a string with zero characters. `" "` (whitespace) is a string with one or more space characters. Neither is the same as `None`, which represents the absence of a value.
...
### Modules
Modules are reusable, importable code (e.g. `math` gives you `abs()`, `round()`, `floor()`).

A type's methods only work on that type: `str` methods (`.upper()`) don't work on `int`, and vice versa (`(50).to_bytes()` isn't valid on a string, `len()` isn't valid on an `int`).
...
| Type | `type()`, `str()` |
| Math | `len()`, `.count()` |
| Transform | `.replace()`, `+`, `f"{}"`, `.split()`, `*`, slicing `[0]` / `[1:3]` |
| Clean | `.lstrip()`, `.rstrip()`, `.strip()`, `.lower()`, `.upper()` |
| Search | `.startswith()`, `.endswith()`, `.find()`, `in` |
| Validate | `.isalpha()`, `.isnumeric()` |
...
### 6.3 Transform — `.replace()`
Replacing with `""` is a handy way to strip out unwanted characters.
```python
price = "$1,299.99"
...
### 6.4 Concatenation (`+` and f-strings)
```python
first_name = "Scott"
...
f-strings (`f"..."`) are the modern, cleaner way to build strings:
```python
name = "Jack"
age = 34
is_student = False
...
### 6.5 Split — `.split()`
Splits one string into multiple pieces on a separator.
```python
# "Adam-24-USA".split('-') -> ["Adam", "24", "USA"]
...
- **Step `[start:end:step]`:** skips characters as it goes.

**When to use which:** positive index is easiest counting in from the left; negative is easiest counting in from the right. For a long string, pick whichever side is closer to what you want.
...
print(date[0:4:2])  # every 2nd character in the year
```

### 6.8 Cleaning — whitespace strip
...
### 6.9 Case conversion
Useful for normalizing data before comparing/searching it.
```python
...
### 6.10 Challenge — clean a messy record
...
### 6.11 Search — `.startswith()`, `.endswith()`, `in`, `.find()`
`in` is less strict than `.startswith()`/`.endswith()` — it checks anywhere in the string.
...
`.find()` locates the position of a match — great for combining with slicing to make code dynamic:
```python
phone1 = "=49-176-12345"
phone2 = "48-654-57389"
...
### 6.12 Validation — `.isalpha()`, `.isnumeric()`
Useful for catching invalid input before it goes further.
```python
country = "USA"
...
- `float` — decimals: `3.15`, `-0.5`, `100.00`
- `complex` — real + imaginary numbers (advanced math/engineering/physics)
...
| Advanced math | `sqrt()`, `sin()`, `cos()`, `log()` |
